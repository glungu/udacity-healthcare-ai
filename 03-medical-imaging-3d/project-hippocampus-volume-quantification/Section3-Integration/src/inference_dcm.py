...
def create_report(inference, header, orig_vol, pred_vol):
    ...
    draw.multiline_text(
        ...
        (
            ...
            f"Hippocampus Anterior Volume: {inference['anterior']}\n"
            f"Hippocampus Posterior Volume: {inference['posterior']}\n"
            f"Hippocampus Total Volume: {inference['total']}"
        ),
    ...
    # img_nd = np.flip((slice_orig/np.max(slice_orig))*0xff).T.astype(np.uint8)
    # This is how you create a PIL image from numpy array
    # img_pil = Image.fromarray(img_nd, mode="L").convert("RGBA").resize((500, 500))
    # Paste the PIL image into our main report image object (pimg)
    # pimg.paste(img_pil, box=(10, 280))
    
    for i,num in enumerate(slice_nums):
        slice_orig = orig_vol[:, :, num]
        slice_pred = pred_vol[:, :, num]
        img_comp = create_alpha_composite(pimg, slice_orig, slice_pred)
        pimg.paste(img_comp, box=(10 + 510*i, 280))

    return pimg


def create_alpha_composite(pimg, slice_orig, slice_pred):
    # original image from ndarray    
    img_nd = np.flip((slice_orig/np.max(slice_orig))*0xff).T.astype(np.uint8)
    img_pil = Image.fromarray(img_nd, mode="L").convert("RGBA").resize((500, 500))
    
    # prediction image from ndarray (red & alpha channels according to predicted label)
    dm = (slice_pred.shape[1], slice_pred.shape[0], 4)
    img_pred_nd = np.zeros(dm, dtype=np.uint8)
    img_pred_nd_red = np.flip(slice_pred*70).T.astype(np.uint8)
    img_pred_nd[:, :, 0] = img_pred_nd_red
    img_pred_nd[:, :, 3] = img_pred_nd_red
    img_pred = Image.fromarray(img_pred_nd, mode="RGBA").resize((500, 500))
    
    # compose 
    return Image.alpha_composite(img_pil, img_pred)
...
def get_series_for_inference(path):
    ...
    print(f'[get_series_for_inference] Path: {path}')
    dicoms = [pydicom.dcmread(os.path.join(path, f)) for f in os.listdir(path)]
    print(f'[get_series_for_inference] Dicoms: {len(dicoms)}')
...
#     series = {}
#     for d in dicoms:
#         k = d.SeriesDescription
#         if k not in series:
#             series[k] = []
#         series[k].append(d)
#     print(f'Unique series: {series.keys()}')
    
#     for k in series:
#         imtype = series[k][0].ImageType
#         print(f'Series: {k}, ImageType: {imtype}, DERIVED: {"DERIVED" in imtype}')
#         if k == 'HippoCrop':
#             print(f'HippoCrop Example: \n{series[k][0]}')

    series_for_inference = [d for d in dicoms if 'DERIVED' in d.ImageType]
    ...
    print(f"Figured out what series to run inference on: {series_for_inference[0].SeriesDescription}")
...
if __name__ == "__main__":
    ...
    report_save_path = r"/home/workspace/out/HIPPOAI_" + header.SeriesDescription + "_" + dt + ".dcm"
    ...
    os_command(f"storescu -aet HIPPOAI localhost 4242 {report_save_path}")
    ...
    time.sleep(2)
    print(f'Report sent to Orthanc, study dir to be deleted...')
